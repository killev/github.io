...
И вот тут то у меня и возник спор с одним знакомым, из которого и родилась эта статья. Какими фичами из представленых 3-х (4-х) выше должен обладать язык чтобы он считался ООП. Ну, или без каких фич он становится бесполезен.

Дальше я расскажу свое личное мнение по поводу 3-х первых.
### Инкапсуляция
Имхо, это самая бесполезная фича из всех. Во-первых, так или иначе, ее постоянно приходися обходить. Либо с помощью переделки хедеров для специальной сборки в с++. Либо с помощью рефлексии.. Либо... И, поверьте, дело не в дизайне! Дело скорее в том, что есть часть клиенткого кода которая должна видеть внутренности объекта. А некоторая - нет. Например отножение db <-> entity. Весь клиентский код должен видеть etity как immutable object. Т.е. видеть только getter. Поменять его можно только через изнутри, через db. И вот тут db должна видеть еще и setter. Опана!
...
### Полиморфизм
Момним, что далее речь идет только о **динамическом полиморфизме**
И вот тут самое интересное. 

Большиство (если не все) строго-типизированные языки реализуют полиморфизм через наследование. Т.е. языку надо так или иначе знать тип объекта, чтобы выяснить какими методами он обладает и можем ли мы получить от него то, что нам надо. И полиморфизм возникает при совпадении двух условий:
1. Наличие у родителя хотя бы одного виртуального метода
2. Переопределение хотя бы одним из наследников хотя бы одного виртуального из виртуальных методов класса родителя. 

Т.е. существует четкая сформированная парадигма, что мы можем переопределять код только виртуальных функций которые появляются как раз при наследовании.. 

Однако. Есть еще языки и нетипизированные. Когда мы вобще не знаем какой объект мы передаем в функцию. И включается механизм "утиной типизции". Если у этого объекта, какого бы типа он ни был на самом деле, есть нужные функции - то мы можем его ипользовать. И это тоже полиморфизм. И, заметте, ни слова про наследование.
