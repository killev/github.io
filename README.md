...
doSomething(SomeObject2())
```
В данном случае функции совсем не обязаны быть виртуальными. Какая именно функция будет вызвана - определяется уже в момент компиляции, т.к. компилятору точно извествен тип подставляемго объекта.В разных языках эта фича называется по-разному. В .NET и Swift - Generics,  в C++ - templates. При этом никаких ограничение по поводу виртуальности функций этот вид полиморфизма не накладывает.  **Templates** в **C++** вобще не обязывают задавать какую бы то ни было начальную структуру используемого класса. Но все-таки **статический полиморфизм** - он не совсем про ООП. Поэтому дальше я буду говорить только про **динамический**.
...
В общем. В Python, например, нет инкапсуляции на уровне языка. Но есть договоренность "условно скрытые" методы начинать с префикса \_\_. Вызывать то мы их можем, но уже за последствия отвечаем сами. И при этом язык прекрасно себя чувствует и развивается.
...
Однако. Есть еще и нетипизированные языки. Когда мы вобще не знаем какой тип объект мы передаем в функцию. И включается механизм так называемой "утиной типизции". Если у этого объекта, какого бы типа он ни был на самом деле, есть нужные функции - то мы можем его ипользовать. А наличие этих функци производится динамически перед самим вызовом. Что, с одной стороны делает разработку более опасной ибо пока программа не дойдет до нужно строчки - никто не знает обладает ли объект нужноq функцией или нет, с другой - более гибкой ибо вы не ограничены иерархиями классов и используете наследование исключительно для переиспользования когда. Правда любители нетипизированных языков с сарказмом говорят, что нет никакой разницы, почему программа упадет: потому что нет функции или потому что нет данных.

И это тоже полиморфизм. И, заметте, ни слова про наследование и виртуальные функции.

Более того, с развитием замыканий использовать callback вместо делегатов стало удобнее. И роль полиморфизма заметно снизилась. Все больше и больше библиотек Java поддерживают колбеки вместо старых Event. Что, правда, не сделало язык удобнее (да простят меня все любители Java).

## Итак.

На самом деле, едиственная фича ООП которая действительно полезна и незаменима - это полиморфизм. Наследование же необходимо только для строго типизированных языков и то, потому что через него реализуется полиморфизм. Тем не менее, несмотря на то, что роль ООП в последнее время сильно снизилась, он остается незаменимым инструментов разработкаи ПО, как единственный используемый на сегодняшний день инструмент моделирования реального мира в коде!
