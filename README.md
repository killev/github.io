...
Основаная прикладная задача, которую решал ООП - это отображение реального мира в программный код. Структурами и процедурами думать сложно. Гораздо проще думать объектами. Вместо: "прочитай количество строк в этом файле", проще мыслить как: "файл, скажи мне количество строк".
...
Почему же сообщения? Такой подход делает не важным местоположение конкретного объекта. Он может быть в другом потоке, процессе, на другом компьторе, в другой вселенной! Не важно. Лишь бы система перезвонила (доставила) нам ответ. И это свойство доает возможность лекго масштабировать систему горизонтально, добавляя ЦПУ когда это необходимо и уирая, когда не надо. И соединяя все это как нибудь. Например сетью.

Но реальность внесла свои коррективы. Оказалось, что передача сообщений требует от системы сложных диспатчеров и транспортов. Что чрезмерно нагружало, и так не очень быстрые, ЦПУ того времени, и требовало дополнительную память, которой и так было не много. Да и не было и нет никакой гарантии, что к тому времени, как к нам придет ответ, он еще будет иметь какой-то смысл. Поэтому, идея с сообщениями на начальном этапе развития не получила.
...
Как я уже сказано выше, ООП - это, в первую очередь, инструмент более естественно отображения (моделирования) реального мира в код. Так появились собственно классы (как описание) и объекты (как физические сущности). 
...
```
А уже какой тип и как их определит - это личная проблема уже данного класса.
Сказонное выше относится к т.н. **динамическому полиморфизму**. Это когда конкретная реализация функций будет определена только в момент вызова функции. Хочу отметить, что я специально здесь не говорю ни о иерархии, ни о виртуальности функций.
...
И вот тут самое интересное. Это единственное свойство ООП, с которым согласны все сторонники ООП. А как же наследование. Ведь полиморфизм реализвуется как раз через наследование. 

Это только отчасти правда. Большиство (если не все) строго-типизированные языки реализуют полиморфизм через наследование. Такому языку надо так или иначе знать тип объекта, чтобы выяснить какими методами он обладает и можем ли мы получить от него то, что надо. И полиморфизм возникает при совпадении двух условий:
...
Т.е. существует четкая сформированная парадигма, что мы можем переопределять код только виртуальных функций, и только после наследование... 

Однако. Есть еще языки и нетипизированные. Когда мы вобще не знаем какой ти объект мы передаем в функцию. И включается механизм  так называемое "утиной типизции". Если у этого объекта, какого бы типа он ни был на самом деле, есть нужные функции - то мы можем его ипользовать. И это тоже полиморфизм. И, заметте, ни слова про наследование.
